...
router.put("/addFriend/:id", fetchuser, async (req, res) => {
  try {
    let userId = req.body.user;
    if (userId.data) userId = userId.data;

    const useDetails = await User.findById(userId);

    if (useDetails.Friends.includes(userId)) {
      return res.json("Already Friend!");
    }
